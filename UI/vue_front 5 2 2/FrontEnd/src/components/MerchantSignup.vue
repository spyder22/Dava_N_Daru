<template>
    <div>
        <div > <Navbar/></div>
        <form>
            <h3 class="head">Merchant Sign Up</h3>
        <div class = "new">
...
export default {
  ...
  methods: {
    ...
    addUser () {
      let result = axios.post('http://localhost:8085/merchant/register', {
        merchantName: this.merchantName,
        merchnatEmail: this.merchnatEmail,
        password: this.password
      })

      console.log(result)
      console.log('I got clicked ! ->', this.userName, this.email, this.password, this.cpassword)
    }
  },
  async login () {
