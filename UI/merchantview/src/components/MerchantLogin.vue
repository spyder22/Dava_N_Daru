        ...
        <div> <Navbar/></div>
            ...
            <div class = "new">
                <div class="form-group">
                    ...
                    <input type="username" name="userName" v-model="merchantName" class="form-control form-control-lg" />
                </div>

                <div class="form-group">
                    <label>Email address</label>
                    <input type="email" name="email" v-model="merchantEmail" class="form-control form-control-lg" />
                </div>
...
export default {
  ...
  methods: {
    checkUser () {
      console.log('I got clicked ! ->', this.merchantName, this.merchantEmail, this.password)
    },
    async checkUserdetails () {
      //e.preventDefault()
      console.log(this.merchantName, this.password)
      const body = {
        email: this.merchantEmail,
        password: this.password
      }
      await axios
        .post(' http://localhost:8085/merchant/signin ', body).then((res) => {
          console.log('yes i got u')
          console.log(res.data)
          localStorage.setItem('name', this.merchantName)
          localStorage.setItem('email', this.merchnatEmail)
          swal({
